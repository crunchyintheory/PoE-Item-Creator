...
export class ValuePipe implements PipeTransform {
  ...
  private replacements: [RegExp, string][] = [
      ...
      [/%%(?<tag>chaos|fire|cold|lightning|crafted|crucible|fractured|foulborn|sanctified|vestigial)%%(.*?)%%\k<tag>%%/gi, '<span class="prop-$1">$2</span>'],
      ...
      [/%%color(?<color>#[a-fA-F0-9]{3,8})%%(.*?)%%color\k<color>?%%/gi, '<span class="prop-color" style="color: $1">$2</span>'],
      [/%%digits%%(.*?)%%digits%%/gi, '<span class="digits">$1</span>'],
  ];
  ...
  transform(value: any, args?: any): any {
      ...
      if(typeof text == "string") {
          text = (text as string).replaceAll(/<\/?|>/g, "%%");
      ...
      if(value.type?.className === PropertyType.StatReq.className) {
          text = text.replace(/((?:\(?[\d-]\)?)+)/g, '%%digits%%$1%%digits%%');
      }
      if(value.type?.className.includes(PropertyType.Veiled.className)) {
          return "";
      }
      else {
          ...
          text = this.sanitizer.bypassSecurityTrustHtml(t); // We have just sanitized the text and replaced the contents with known-safe fixed templates.
      }
